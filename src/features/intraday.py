...
def build_done_detail_features(done: pd.DataFrame) -> pd.DataFrame:
    df = done.copy()
    df['ts_sec'] = df['timestamp'].dt.floor('s')
    df['buy_aggr_lot'] = np.where(df['side_aggressor'].eq('BUY'), df['lot'], 0)
    df['sell_aggr_lot'] = np.where(df['side_aggressor'].eq('SELL'), df['lot'], 0)
    sec = df.groupby(['trade_date','ticker','ts_sec'], as_index=False).agg(
        trades_in_sec=('lot','size'), lot_in_sec=('lot','sum'), sec_high=('price','max'), sec_low=('price','min'),
        buy_aggr_lot_sec=('buy_aggr_lot','sum'), sell_aggr_lot_sec=('sell_aggr_lot','sum')
    )
    ...
    pair_sum = pair.groupby(['trade_date','ticker'], as_index=False).agg(pair_total_lot=('pair_lot','sum'), top_pair_lot=('pair_lot','max'), active_pairs=('pair_lot','size'))
    ...
    daily = df.groupby(['trade_date','ticker'], as_index=False).agg(
        trade_count=('lot','size'), total_lot=('lot','sum'), buy_aggr_lot=('buy_aggr_lot','sum'), sell_aggr_lot=('sell_aggr_lot','sum'),
        first_trade_price=('price','first'), last_trade_price=('price','last'), avg_lot=('lot','mean'), max_lot=('lot','max')
    )
    burst_sum = sec.groupby(['trade_date','ticker'], as_index=False).agg(
        burst_count=('burst_flag','sum'), buy_sweep_count=('buy_sweep_flag','sum'), sell_sweep_count=('sell_sweep_flag','sum'), max_trades_same_sec=('trades_in_sec','max')
    )
    ...
    daily['tape_conviction_score'] = (
        daily['buy_aggr_ratio'] * 55 + (daily['buy_sweep_count'] / (daily['buy_sweep_count'] + daily['sell_sweep_count'] + 1)) * 25 + (1 - daily['top_pair_share'].fillna(0.5)) * 20
    ).clip(0,100)
...
def build_orderbook_features(book: pd.DataFrame) -> pd.DataFrame:
    df = book.copy()
    bid_cols = [f'bid_{i}_lot' for i in range(1,6)]
    off_cols = [f'offer_{i}_lot' for i in range(1,6)]
    ...
    daily = df.groupby(['trade_date','ticker'], as_index=False).agg(
        top3_bid=('top3_bid','mean'), top3_offer=('top3_offer','mean'), weighted_bid=('weighted_bid','mean'), weighted_offer=('weighted_offer','mean'), imbalance=('imbalance','mean'), spread_bps=('spread_bps','mean')
    )
...
def build_intraday_features(done: pd.DataFrame, book: pd.DataFrame) -> pd.DataFrame:
    d = build_done_detail_features(done)
    b = build_orderbook_features(book)
    out = d.merge(b, on=['date','ticker'], how='outer')
    out['microstructure_strength_score'] = (
        0.35 * out['tape_conviction_score'].fillna(50) + 0.35 * out['book_support_score'].fillna(50) + 0.15 * (100 - out['transfer_suspicion'].fillna(50)) + 0.15 * (100 - out['spoof_risk_score'].fillna(50))
    ).clip(0,100)
    out['microstructure_weakness_score'] = (100 - out['microstructure_strength_score']).clip(0,100)
    return out
